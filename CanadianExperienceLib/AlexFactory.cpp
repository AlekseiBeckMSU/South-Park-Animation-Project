...
/**
 * This is a factory method that creates our Alex actor.
 * @param imagesDir Directory that contains the images for this application
 * @return Pointer to an actor object.
 */
std::shared_ptr<Actor> AlexFactory::Create(std::wstring imagesDir)
{
    shared_ptr<Actor> actor = make_shared<Actor>(L"Alex");

    auto shirt = make_shared<ImageDrawable>(L"Shirt", imagesDir + L"/alex_shirt.png");
    shirt->SetCenter(wxPoint(44, 138));
    shirt->SetPosition(wxPoint(0, -114));
    actor->SetRoot(shirt);

    auto vest = make_shared<ImageDrawable>(L"Vest", imagesDir + L"/alex_vest.png");
    vest->SetCenter(wxPoint(44, 138));
    shirt->AddChild(vest);

    auto lleg = make_shared<ImageDrawable>(L"Left Leg", imagesDir + L"/alex_jeans_leftr.png");
    lleg->SetCenter(wxPoint(11, 9));
    lleg->SetPosition(wxPoint(27, 0));
    shirt->AddChild(lleg);

    auto rleg = make_shared<ImageDrawable>(L"Right Leg", imagesDir + L"/alex_jeans_leftl.png");
    rleg->SetCenter(wxPoint(39, 9));
    rleg->SetPosition(wxPoint(-27, 0));
    shirt->AddChild(rleg);

    auto headb = make_shared<ImageDrawable>(L"Head Bottom", imagesDir + L"/alex_headb.png");
    headb->SetCenter(wxPoint(44, 31));
    headb->SetPosition(wxPoint(0, -130));
    shirt->AddChild(headb);

    auto headt = make_shared<HeadTop>(L"Head Top", imagesDir + L"/alex_headt.png");

    // sets current head to alex head instead of harold
    headt->SetCurrentHead("AlexHead");

    headt->SetCenter(wxPoint(55, 109));
    headt->SetPosition(wxPoint(-15, -31));
