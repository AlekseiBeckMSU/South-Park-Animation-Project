...
{
    // Create PolyDrawable object
    PolyDrawable polydrawable(L"Alex");

    // Test to see if constructor and getter set and return "Alex"
    ASSERT_EQ(L"Alex", polydrawable.GetName());
}
...
TEST(PolyDrawableTest, Color)
{
    ...
    // Assert that the default color value is black while also testing getter
    ASSERT_EQ(*wxBLACK, polydrawable.GetColor());

    // Set PolyDrawable object's color to a different color, blue
    polydrawable.SetColor(*wxBLUE);

    // Assert that the SetColor and GetColor functions work by seeing if *wxBLUE is returned
    ASSERT_EQ(*wxBLUE, polydrawable.GetColor());

    // Set PolyDrawable object's color to a different color, yellow
    polydrawable.SetColor(*wxYELLOW);

    // Assert that the SetColor and GetColor functions work by seeing if *wxYELLOW is returned
    ASSERT_EQ(*wxYELLOW, polydrawable.GetColor());
}
